...
class EgressProcessor(ScheduledModuleThread, InternalMsgQ):
    ...
    def _read_config(self):
        ...
        try:
            self._signature_user = Conf.get(SSPL_CONF, f"{self.PROCESSOR}>{self.SIGNATURE_USERNAME}",
                                                                 'sspl-ll')
            self._signature_token = Conf.get(SSPL_CONF, f"{self.PROCESSOR}>{self.SIGNATURE_TOKEN}",
                                                                 'FAKETOKEN1234')
            self._signature_expires = Conf.get(SSPL_CONF, f"{self.PROCESSOR}>{self.SIGNATURE_EXPIRES}",
                                                                 "3600")
            self._producer_id = Conf.get(SSPL_CONF, f"{self.PROCESSOR}>{self.PRODUCER_ID}",
                                                                 "sspl-sensor")
            self._message_type = Conf.get(SSPL_CONF, f"{self.PROCESSOR}>{self.MESSAGE_TYPE}",
                                                                    "Alerts")
            self._method = Conf.get(SSPL_CONF, f"{self.PROCESSOR}>{self.METHOD}",
                                                                    "Sync")
        ...
        try:
            ...
            if self._jsonMsg.get("message").get("actuator_response_type") is not None and \
                ...
                self._jsonMsg.get("message").get("actuator_response_type").get("thread_controller").get("thread_response") == \
                    ...
                    logger.info("EgressProcessor, _transmit_msg_on_exchange, received" \
            ...
            # Routing requests for IEM msgs sent from the LoggingMsgHandler
                ...
                if self._iem_route_addr != "":
                    ...
                    logger.warn("EgressProcessor, Attempted to route IEM without a valid 'iem_route_addr' set.")
            else:
                self._add_signature()
                try:
                    ...
                    logger.error(f"EgressProcessor, _transmit_msg_on_exchange, error {e} in producing message,\
                                    adding message to consul {self._jsonMsg}")
                    self.store_queue.put(jsonMsg)
                except Exception as err:
                    logger.error(f'EgressProcessor, _transmit_msg_on_exchange, Unknown error {err} while publishing the message, adding to persistent store {self._jsonMsg}')
                    self.store_queue.put(jsonMsg)
            ...
            logger.error(f'EgressProcessor, _transmit_msg_on_exchange, problem while publishing the message:{ex}, adding message to consul: {self._jsonMsg}')
